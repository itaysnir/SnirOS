...
int sys_send_packet(char *buf, uint32_t size);
int sys_recv_packet(char *buf, uint32_t size);
int sys_get_mac(char *buf);
int sys_enable_non_blocking_socket();
int sys_disable_non_blocking_socket();
int sys_set_input_envid(int32_t envid);
int sys_add_filter(uint32_t ip, uint16_t port);
...
int     jpf_add_filter(uint32_t ip, uint16_t port);
